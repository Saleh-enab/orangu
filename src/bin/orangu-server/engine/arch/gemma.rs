...
//! forward paths ([`GemmaModel::run_layers_cpu`]) — the matmuls still
//! dispatch to the GPU backend, but the fully-fused single-submission
//! decode/replay Vulkan paths ([`GemmaModel::record_one_sequence_decode`] etc.)
...
gemma 4 checkpoint."
    ...
    /// this request's own `SlotGuard::id() + 1`; a shared constant here
    /// would let two `slots > 1` requests decoding concurrently corrupt
    /// each other's cached GPU buffers) (one
    ...
    /// TIMESTAMPS=1`): `timestamp_query_set`'s own `wgpu::QuerySet` is sized
    /// for exactly one sequence's `n_layer + 3` boundary points, with no
    /// batch dimension, and a shared query set written from *M*
    /// concurrently-
