...
Enter it with the `/auto_review` command, or the natural-language form `auto review`. Give it a file — `/auto_review <file>` — to review just that one file instead of the whole branch (see *Reviewing a single file* below), a glob pattern — `/auto_review src/main/java/**` — to review every file it matches (see *Reviewing files by pattern* below), or the `all` keyword — `/auto_review all` — to review every file in the project instead (see *Reviewing every file* below). The view opens in a **pre-start phase** that waits for you to begin the run (see *Starting the run* below); add the `immediate` keyword — `/auto_review immediate` (or `/auto_review <file> immediate`, or `/auto_review all immediate`) — to skip it and start at once. Add the `deep` keyword — `/auto_review deep` — to start every file in **Deep** mode instead of Normal, the same Deep the Alt+m pre-start cycle offers per file (see *Normal, Deep, and Ignore* below), applied to the whole launch at once. The three keywords and a file or pattern argument combine freely, in any order — `/auto_review src/main/java/** immediate`, `/auto_review deep src/**/*.java`, and the longest accepted form, `/auto_review deep all immediate` (or, natural-language, `auto review deep all immediate`).
...
### Reviewing files by pattern

`/auto_review <pattern>` reviews every file matching a glob pattern — `/auto_review src/main/java/**` reviews everything under `src/main/java`. A file argument is a pattern whenever it contains a `*`, `?`, `[`, or `{`; otherwise it is a single file as above. The pattern generalizes the single-file argument, so what gets reviewed follows the same rule: on `main`/`master` every Git-tracked file the pattern matches is reviewed **whole** (a full read of its current content; untracked and `.gitignore`d files never match); on any other branch only the **changed** files it matches are reviewed, as their diff against the default branch, under the same rebased-branch guard. The view, the categories, and the report are the same as a whole-branch run — the checklist just holds the matching files.

Patterns follow `.gitignore` rules: `*` and `?` never cross a `/` (`src/*.rs` is only the direct children of `src`), `**` does (`src/**/*.rs` is every `.rs` file under `src` at any depth, and `src/main/java/**` everything under that directory), `[a-z]` and `{tui,cli}` work as usual, and a pattern **without** a `/` — `*.java`, `Foo?.rs` — matches on the file's **name** at any depth, the same way a hand-typed bare `tui.rs` resolves. Paths match as `git` reports them, repository-relative with `/` separators; a leading `./` is ignored. A pattern matching nothing is refused — `No tracked file matches '<pattern>' to review.` on `main`/`master`, `No changed file matches '<pattern>' against <base>.` on a branch — and a malformed one with `Invalid pattern '<pattern>': …`. The `immediate` and `deep` keywords combine with a pattern in any order — `/auto_review src/main/java/** immediate`, `/auto_review deep src/**/*.java immediate` — and the natural-language form takes one too: `auto review src/main/java/**`. If `all` is given alongside a pattern, `all` wins.

### Reviewing every file

`/auto_review all` reviews every file `git` tracks in the project instead of the branch diff, a single file, or a pattern — the view, the categories, and the report are exactly the same, just with every tracked file in the checklist. Each file gets the same treatment as a single-file review on `main`/`master`: a full read of its current content, every line in scope, regardless of which branch you are on — `all` reviews what is actually on disk, not a diff, so a branch behind the default branch is reviewed anyway (the rebased-branch guard does not apply here). Untracked files and anything excluded by `.gitignore` are left out — the file list comes straight from `git ls-files`. If `all` is combined with a file or pattern argument, `all` wins and the file is ignored; combine it with `immediate` — `/auto_review all immediate` — to skip the pre-start phase and begin reviewing the whole project at once. The natural-language form takes it too: `auto review all`.
...
Each request runs in its own scratch exchange, **without tool definitions** and with a **capped response length** (`[orangu].review_max_tokens`, default `512`; `0` disables the cap) — a review can neither wander off into tool calls nor generate unbounded output, which keeps single requests fast and bounded even on slow local models. For deeper reviews with a thinking model, raise the cap (e.g. `2048`) so the thinking tokens do not eat the answer; the *Response-token caps* part of the Configuration chapter covers the trade-offs in depth. Every category prompt carries the whole file plus its diff (not just the diff), and — on an orangu-server — a file's category requests are all pinned to the same `id_slot`, so its file and diff are prompt-processed once and reused from the server's KV cache across every category instead of being recomputed each time. On a 9k-token file that reuse is the difference between about a minute and about five seconds per category.

Large files are the exception, and deliberately: a file over 32 KiB (or 4,000 lines) is reviewed from its **diff alone**, without the whole-file section. Prompt cost does not scale linearly — attention is quadratic, so the rate at which a server processes a prompt falls as the prompt gets longer, and a single 100 KiB source file can cost more to send than the rest of a change set put together. Reviewing a 37-file change with the cap in place is less than half the prompt tokens of doing it without.

If the server refuses a prompt anyway — its device holds less context than that, which varies by machine — the file is retried once without its whole-file section and with its diff bounded, and reviewed from that rather than recorded as a failed request. The omitted part of a bounded diff is marked inline, so the model is not left to read a truncation as the end of the change. The reviews are independent of each other and nothing is added to your chat session.
...
The same command handles conflicts from an in-progress merge, rebase, or cherry-pick. It asks Git for every unmerged path and gives the complete list directly to the model; conflict paths are never compressed. A live conflict is enough to run `/create_patch`; no review report is required. When a report and conflicts both exist, the model handles both in one pass. It is instructed to reconstruct the intended combined behavior from the base, both sides, surrounding code, and tests; and to explain, per conflict, what each side changed and why the resolution is right — that explanation is what you read. The model does not edit the conflicted files or run Git itself: it writes each resolution out as a block, and orangu applies it, stages the file, and continues the operation — see below.

The prompt carries the evidence rather than sending the model to look for it. It names the operation in progress and what the two sides of the markers mean — during a rebase `<<<<<<< HEAD` is the base being rebased onto and the `>>>>>>>` side is the commit being replayed, the reverse of a merge, which is the classic way to resolve a conflict the wrong way round — and for each unmerged path it shows every conflict region as it stands on disk, with its markers, eight lines of context either way, and the same line numbers `show_file` reports, so the model can edit the region with `modify_file` at once. The excerpts share a budget of about 24 000 characters; a conflict larger than that is described by its line ranges for the model to read back. A path with no markers on disk (a delete/modify or binary conflict) is described as such instead.

Each resolution comes back as a block naming the path and the conflict's own line range:

```text
=== RESOLVED src/libpgmoneta/utils.c lines 4258-4262 ===
      ws = pgmoneta_append(ws, WORKSPACE_DEFAULT);
=== END RESOLVED ===
```

orangu replaces the lines from `<<<<<<<` through `>>>>>>>` with the block's content — refusing a range that is no longer a conflict block, so a drifted number never overwrites code — and stages the file with `git add`; a code fence or copied line numbers inside the block are tolerated. A model that edited the file with a tool anyway is fine too: a path with no markers left is left as it is.

When the model has reported, orangu finishes the operation itself: with no unmerged paths left it runs `git rebase --continue` (or `git merge --continue`, `git cherry-pick --continue`), keeping the commit message as it is, and reports `Rebase finished: branch '<name>' is at <commit>` — so the branch comes out whole rather than parked mid-rebase on a detached HEAD. If no block was given for a conflict it says which path and lines are still unresolved and stops without continuing, and if the next commit of a rebase conflicts too it stops there and says so; in both cases `/create_patch` again picks up where it left off. This tail runs only when the model's turn completed, not when it was cancelled or failed, and it follows the turn even when a tab switch sent it to the background.

Taking another user's branch through to a clean, rebased copy is therefore:
