...
- `engine/vecdot.rs` — the fused `int8` CPU kernels: a dot product taken
  ...
  entirely. See below.
- `engine/hadamard.rs` — Prism's `prism.hadamard.*` fold (the
  `Ternary-Bonsai-2` releases): the metadata and its validation, the
  activation-side blockwise Walsh–Hadamard rotation with its sign vector
  and the delta-net head regrouping, the inverse for embedding rows, and
  the `FoldLedger` an architecture's loader uses to prove every folded
  weight in the file goes through a transform site.
...
- `qwen35.rs` — Qwen3.5-family dense (confirmed against upstream
  ...
  it looks like. Prism's `Ternary-Bonsai-2-27B` is this architecture with
  Hadamard-folded weights: the trunk carries a `hadamard::Rotation` beside
  every folded projection (one per shared input) and `DenseFfn` rotates the
  SwiGLU intermediate into `ffn_down`'s basis; its `PQ2_0`/`PTQ1_0`
  tensors are ordinary types to `quant`/`vecdot`/the Vulkan shaders.
  What this trunk still lacks against `gemma.rs` is a device-resident
  decode chain: every projection goes through `Backend::matmul_batch`, so
  a token is ~250 round trips of ~3 ms fixed cost each on the development
  board's Mali, against kernels that would stream the 5.5 GiB in ~0.25 s.
  Closing that gap is `gemma.rs`'s `record_decode_forward` shape — one
  encoder per chunk of layers, activations kept on the device — plus what
  the hybrid layer adds: a `conv_step` + delta-rule + gated-norm kernel
  for the recurrent sub-layer, and the Hadamard rotation as a workgroup
  FWHT so the activations need not come back to the host for it.
...
**Fitting a context** (`--context <TOKENS>`, `ORANGU_CONTEXT`,
`[orangu-server].context`). Unset, every layer goes to the card when the
weights fit and the context is whatever the card has left after them —
the pool's share of the headroom, sized from the driver's budget — and
a longer prompt is refused. Set, the KV cache that many tokens need (in
the device's own storage width, `n_layer · n_head_kv · head_dim · 2` per
token) plus the prefill's scratch reserve is taken out of the head card's
budget *before* the layers are placed, so a model whose weights fit the
card with no room for that context has its last layers moved to the host
until there is (`device_split = cpu`, reported at startup with the plan).
It is a choice rather than a default because it costs decode: a layer on
the host is read at the host's rate every token, and the refusal is what
keeps a model that fits the card at its full rate. Measured on a 4 GiB
card with a 3.2 GiB `Q8_0` model asked for 8192 tokens: 12 of 28 layers
to the host, 9344 tokens of context where there were 1728, decode 43 →
9 tok/s at depth 0. A model already split for its size gets the same
reserve on top of its placement. The card's other tenants count: what
the compositor holds is planned around, not over.

**Windowed layers keep a window.** A gemma-4 file's sliding-window layers
— five of every six — never attend further back than their window (512
or 1024 tokens), so their device copy is a ring of the window plus one
prefill chunk rather than one row per position, whatever the context:
`kv_elements_per_token` counts only the full-attention layers, and the
rings are a fixed charge per slot taken out of the pages' budget first.
The host copy keeps every position, so rollback, prefix reuse and slot
save are unchanged, and on a paged model the pool holds those layers'
rows for sharing while allocating them no device pages. A layer that
shares another's cache holds nothing at all. Together on a 4 GiB card:
E4B's ceiling 2944 → 15424 tokens, and `--context 8192` needs no layer
moved. `ORANGU_SWA_FULL=1` keeps a full-context copy on every layer, for
comparison.
...
**The host tier's decode.** Three things bound a split model's decode
once most of its layers are on the host, and each was measured on a
48-layer model split 13:35 over a 4 GiB card and the host (5.7 tok/s
against 4.3 before, level with the reference engine):

- **The row dot's rate.** `vecdot::dot_k_row` read `Q4_K` at 7 GB/s a
  core and 30 at eight where a plain sum of the same bytes reads 18 and
  35: the per-32 form unpacks nibbles into an `i8` array and pays a
  horizontal sum per 32 elements. The super-block-wide AVX2 kernels
  (`dot_k_row_q4_k_avx2` and the `Q5_K`/`Q6_K`/`Q8_0`/`Q4_0` forms) take
  the four 32-byte runs as nibble bytes straight into `vpmaddubsw`, widen
  by the scales with `vpmaddwd`, and keep eight float lanes across the
  row — one horizontal sum per row. 8 GB/s a core, **35 at eight**, the
  memory's own rate.
- **A layer's Q/K/V as one parallel region** (`CpuBackend::matmul_batch`
  at one token). As three regions the three matvecs of 8.8, 4.4 and 4.4
  MB each paid a ramp, a tail and a join, and read at 18.6 GB/s where the
  gate/up pair read at 29; one region over every op's rows is one ramp
  and one tail (0.96 → 0.70 ms a layer).
- **The card's layers as one chain.** `decode_device_runs` declined the
  fused decode chain for the whole model when any layer had no device
  behind it, so the card's thirteen layers ran as thirteen per-layer
  chains: 26 submissions and 13 readbacks a token. `decode_runs_with_host`
  keeps the host tier as a run of its own, stepped by `run_layers` over
  just those layers, and records every device run as one chain — four
  submissions a token.
- **The host attention split over the pool at decode.** The host kernel
  parallelises across query tokens, and a decode step has one: at a
  thousand positions it read the window's 18 MB of K and V on one core
  (1.05 ms a layer, 37 ms a token over 35 layers). `attention_decode_split`
  gives every (K/V head, 128-position chunk) its own task with its own
  softmax statistics and merges them per head — the shape the device
  kernel has for the same case — 0.72 ms a layer. What is left of the
  depth cost is the host cache's `f32` rows; a half-precision host cache
  would halve the bytes.
- **Planned against the card's free memory.** The fill planned 80% of the
  card's capacity for weights; with a video player and a browser holding
  0.7 GiB of it, the last layers paged per token (1.2 tok/s). The plan now
  subtracts what other processes hold at start (`DeviceCandidate::
  vram_used_bytes`, from the kernel driver's accounting) and says so.
...
| `ORANGU_PREFILL_CHUNK_MS` | `3000` (integer) | Wall-clock target for one prefill submission, **on a backend that has a submission timeout only**. A chunk is timed and the next is sized from what that chunk revealed about this machine's cost, because cost per token climbs with context: a fixed 512-token chunk measured 2.3 s at position 512 and **10.1 s at position 6 656**, past the ~10 s `amdgpu` allows before it resets the device (see *Losing the GPU device*). **It is ignored where there is no device to lose**, and deliberately: the quotient it adapts on is a per-token rate only for a resident model. When weights stream from disk each pass costs about the same whatever it contains, so a narrow chunk reports a huge apparent rate, the next chunk shrinks, and the fixed cost is paid again over fewer tokens. Measured, that does not settle — it lands on the 16-token floor on the *first* chunk and stays: a 1,016-token prompt became **63.8 passes and 78.5 GiB of reads on a 1.23 GiB model, prefilling at 0.8 tok/s**, against one pass, 1.29 GiB and 45.7 tok/s once the adaptation is dropped. |
| `ORANGU_PREFILL_CHUNK_SPREAD` | `4` (integer; `0` turns it off) | How a chunk's wall time is counted against `ORANGU_PREFILL_CHUNK_MS`. The budget guards against one *submission* outliving the driver's timeout, and a chunk is many submissions — a handful on the device-resident stream, one per matmul on a host-orchestrated model. A chunk of `n` submissions counts as `elapsed × spread / n` (its mean submission time with a margin for the longest), never more than `elapsed`; a chunk of `spread` submissions or fewer counts in full, which leaves the stream's chunks exactly as they were. Before this, a model split onto an integrated GPU — where a 128-token chunk takes ~34 s across ~200 submissions — was cut to eleven-token chunks, each paying every layer's fixed cost, and prefilled a 572-token prompt at 1.5 tok/s where one chunk ran at 5.7. `0` counts the whole wall time, the rule this replaces, the control arm. |
...
| `ORANGU_NO_CHUNK_COST_FIT` | unset (the fit is **on**) | Size each prefill chunk from the previous one's rate alone, as this did before the cost of a submission was modelled as `fixed + per_token · n`. The control arm, kept so the change can be measured against what it replaced in one `orangu-bench --sweep` rather than across two sessions. **Why the fit exists:** a submission pays for itself before it processes a token, so reading `elapsed / n` as a per-token rate charges all of that to per-token work — worst on the opening 16-token probe, which is almost entirely fixed cost. Its apparent rate then comes out several times the real one and every following chunk is sized short. Two observations at different widths separate the terms, and the estimate is kept across requests, so only the *first* prompt a server sees pays a ramp. Measured on `gemma-4-E2B-it:Q4_K_M`, a 1,120-token prompt went from 5 submissions (`16, 122, 351, 473, 158`) to 3 (`512, 512, 96`). Both numbers are measured here rather than carried in as constants, which is what makes the sizing portable to a machine with a different round-trip cost. |
| `ORANGU_PREFILL_TRACE` | unset (`1` turns it on) | Every 256 readbacks of one size, prints the wall clock of a device submission's phases, in microseconds each: `alloc` (the staging buffer), `finish` (the encoder's own — the commands recorded since it was opened, translated for the driver; ~7 µs a dispatch on the CIX P1), `submit` (the queue's), `wait` (the device, plus launch and readback latency) and `copy` (the mapped bytes out). The small size is a decode step's; a device idles through `finish` and `submit` unless an earlier submission is still running (see `ORANGU_HYBRID_FLUSH_LAYERS`). |
| `ORANGU_PREFILL_CHUNKS` | unset (off) | Print one line per prefill naming the chunk widths the sizer chose, e.g. `1120 tokens from 0 in 3 submissions: [512, 512, 96]`. Answers "what did the sizer actually do" without `ORANGU_PREFILL_TRACE`, which writes inside the submission loop and therefore changes the cost it is measuring — under it the sizer reads inflated chunk costs and picks narrower chunks, so it cannot be used to check the sizer's own choices. |
...
| `ORANGU_NO_FLOAT_WIDE` | unset (wide float **on**) | Set to serve `F32` and `F16` weights at decode with the generic per-element `reduce` instead of the `vec4`, loads-in-flight kernel — the generic one reads each weight byte by byte through the type's `dequant_element` and each activation alone, one round trip per element. Matters for a model that carries F32 tensors on the decode path (the per-layer-embedding gate and projection of the `gemma` families): both dropped to about half their time. Same arithmetic, different summation order. The control arm of its measurement. |
| `ORANGU_Q6K_V4` | unset (word reads **on**) | Set to have the `Q6_K` light decode kernel read its 210-byte blocks through 16-byte loads with a per-load alignment test, as it originally did, instead of as words with each thread's misalignment resolved once (a thread's blocks are all congruent modulo four). Same values in the same order, so output is byte-identical; about a third faster per byte. The control arm of its measurement. |
| `ORANGU_NO_DEVICE_TOPK` | unset (device top-k **on**) | Set to have a *sampled* decode step (temperature above zero, no grammar, `top_k` in `1..=64`) read the whole vocabulary's logits back and scan them on the host, as it originally did, instead of having the device apply the repeat penalty and return the sampler's `top_k` candidates. The host still applies temperature, `top_p`, `min_p` and draws — from the same candidates in the same order, so the distribution is unchanged. The control arm of its measurement (+15% at temperature 0.8 on the model this was measured on; the client sends no temperature, so this is the path a review takes). |
| `ORANGU_COPY_INPUTS` | unset (direct **on**) | Set to have every fused decode layer *copy* its per-call inputs — the previous layer's output, attention's output, the per-layer-embedding slice — into its own buffers before reading them, as it originally did, instead of binding them where they already are. Each copy is a command outside any compute pass, so each also split the layer's pass. The control arm of its measurement (+5% on the model this was measured on). |
...
| `ORANGU_BLOCK_DOT_WORDS` | on (`0` turns it off) | The block-hoisted kernel's per-type `block_dot` for `Q2_K`, `Q3_K`, `IQ4_XS`, `IQ3_S` and `IQ2_S` as **word reads**: a lane's sixteen elements taken as whole words (one `qs` word and one `hmask` word for the two K-quants, the four 2-bit fields of each byte being four sub-blocks' worth), four activations per load, in straight-line code — where the byte-reading forms decode one byte at a time in a rolled loop, sixteen word loads with a shift and mask each and sixteen scalar activation loads, in order. Measured on the E2B UD-Q2_K_XL file the decode step's device time went from 36 to 15.5 ms (24.8 → 50.5 tok/s); output differs from the byte-reading forms only in summation order. `0` is the control arm, and takes the wide pipelines below with it. |
| `ORANGU_BLOCKS_PER_TRIP` | `1` (1, 2 or 4) | Blocks a lane of the block-hoisted decode kernel takes per trip round its block loop, their loads issued together. Measured over the kernel's own hot weights in isolation, two halves the FFN gate's time; inside a decode step the same op loses 28% at two and 130% at four, so the default is one and the knob is for measurement. |
| `ORANGU_KQ_LIGHT` | unset (off) | The word-reading types' decode mat-vec on the `Q4_K` light kernel's skeleton (32 lanes, sixteen per super-block, two in flight) instead of the block-hoisted one. Measured level with it inside a decode step — −4% on the down projection, +6% on the gate, the token unchanged — which is what it was built to find out. |
| `ORANGU_Q2K_STUB` | unset (`loads` or `alu`) | A **wrong** `Q2_K` decode routine for splitting the kernel's time: `loads` does every load with the arithmetic reduced to a sum of the words, `alu` the full arithmetic on words made up from the lane with no weight loads at all. Timing only; the start-up check retires a wrong kernel, so pair it with `ORANGU_VK_NO_SELFCHECK=1`. Measured on the E2B UD-Q2_K_XL FFN gate: 66 µs real, 49.5 loads only, 65.6 arithmetic only — the kernel is bound by its arithmetic, not the weight stream. |
| `ORANGU_DECODE_MMVQ` | on where the device accelerates the packed 8-bit dot (`0` turns it off) | The decode mat-vec of `Q2_K`, `Q3_K`, `IQ4_XS`, `IQ3_S`, `IQ2_S` and the legacy `Q4_0`, `Q4_1`, `Q5_0`, `Q5_1`, `Q8_0` on the **integer dot**: the weight bytes as stored against an 8-bit activation through `dot4I8Packed`. In the fused decode chain the activation is quantized **by the kernel that produces it** — the post-attention norm pair writes the FFN input's 8-bit form beside the float one for the gate and up projections, the fused activation·multiply writes the product's for the down projection, and the attention norm writes its output's for the Q/K/V projections — so no quantize dispatch and no copy is added and every fusion stays. That is the difference from `ORANGU_Q4K_MMVQ`'s form, a quantize dispatch per projection and the copy-in input path, which lost 3 ms a token to its dispatches and the fusions it gives up. Measured on the E2B UD-Q2_K_XL file: gate and up 65 → 43 µs a layer, down 100 → 63, wq 32 → 28 (the norms' epilogues cost 0.5 and 1.7 µs), the token's device time 14.8 → 11.9 ms; on the `Q4_1` file 18.7 → 13.7 ms (gate/up 112 → 59), on `Q8_0` 19.6 → 18.4. The output projection and the per-layer-embedding ones stay on the float kernels. Output differs from the float kernels by the 8-bit rounding of the activation, as the prefill's integer-dot GEMM does; `0` is the control arm and the float path's numbers. The CPU-orchestrated matmul batch quantizes per op itself. |
| `ORANGU_BLOCK_HOISTED_WIDE` | on (`0` turns it off) | For the word-reading types, a second block-hoisted pipeline at **four rows per workgroup**, taken for a projection of 2048 rows or more: one row per workgroup leaves each workgroup too short-lived for its fixed cost on a wide projection, while on the 256-wide per-layer-embedding projections four rows leave too few workgroups (0.41 → 1.58 ms per token), hence the split by width rather than `ORANGU_REDUCE_N_ROWS` for everything. Measured −12% on the E2B `Q2_K` FFN gate, neutral elsewhere. |
| `ORANGU_TERNARY_IDOT` | on where the device executes `dot4I8Packed` natively (`0` turns it off, `1` insists) | The integer-dot decode kernels for Prism's ternary types (`PQ2_0`, `PTQ1_0`), taken ahead of the block-hoisted words kernel for a projection of 2048 rows or more. The activations are quantized to `int8` per 128-block inside the kernel (a `subgroupShuffleXor` block maximum, `pack4x8snorm`), the trits packed four to a word and dotted with `dot4I8Packed`, the `-1` of a `0/1/2` trit one `Σq` subtraction per block. On the Mali-G720 `PTQ1_0` 7.5 → 16 GB/s at the FFN gate shape, 24 at the down shape; the 27B's Vulkan decode 0.98 → 1.54 tok/s. Off, the float words kernel is the control arm; the cross-checks compare each against its own activation rounding. |
| `ORANGU_TERNARY_I8` | on (`0` turns it off) | With `ORANGU_TERNARY_IDOT`, the integer-dot ternary kernels read the activation already quantized to `int8` by a dispatch per token over the norm's or the previous kernel's output (`shader_source_quantize_q8`'s per-32 layout, the one the `block_hoisted_i8` kernels read), rather than quantizing it inside the kernel per row group. On the Mali-G720 the `PQ2_0` FFN projections are 1.3× faster for it (gate 901 → 698 µs, down 708 → 535; the 27B's decode 3.28 → 3.64 tok/s); `PTQ1_0`, bound by its trit remainder chain, gains nothing. Off, the kernels take the `f32` input — the control arm. |
| `ORANGU_TERNARY_REPACK` | on (`0` turns it off) | On a Vulkan device, a `PTQ1_0` file's weights are repacked as `PQ2_0` when the model loads (`QuantMatrix::repacked_two_bit`): the same trits and scales, two bits an element instead of base-3 packing — lossless, 21% more bytes on the device (the 27B: 5.5 → 6.7 GiB), a few seconds at load. The device's `PTQ1_0` kernel is bound by the trit unpacking where its `PQ2_0` kernel is a shift and a mask, so a `PTQ1_0` file then decodes at the `PQ2_0` rate. The banner's `Kernels` line and the `weights … on device` figure describe the served form (`pq2_0`, the larger size). Off, the file's format is served as is. |
| `ORANGU_TERNARY_IDOT_ROWS` | `8` (`1..=16`) | Rows per workgroup of the integer-dot ternary kernels: the activations are quantized once per block per workgroup and reused for every row. Measured 4 → 8 as 1387 → 1255 µs on the `PTQ1_0` gate projection, 16 no better. |
| `ORANGU_FUSED_ATTN_LAYER` | on (`0` turns it off) | The Qwen hybrid trunk's full-attention sub-layer as one submission at decode (`VulkanBackend::fused_attention_layer`: the projections, per-head norms, RoPE, cache write, attention, sigmoid gate, output fold and `wo`). Off, the sub-layer runs as a batched projection submission, the host's norms/RoPE/attention, and a `wo` submission. Measured level at depth 0 and +3% at depth 512 on the 27B ternary model; the control arm. |
| `ORANGU_HYBRID_FLUSH_LAYERS` | `8` (`0` for one submission) | How many layers of the Qwen hybrid trunk's device token go into one submission: the device runs the layers already submitted while the host records and finishes the next, where one submission per token left it idle through the encoder's `finish` (~7 µs a dispatch, 14 ms for the 27B's token on the CIX P1). On that board `PQ2_0` decode 3.64 → 3.86 tok/s at 8; 16 and 4 are within 1% of it. |
| `ORANGU_HYBRID_DEVICE_TOKEN` | on (`0` turns it off) | The Qwen hybrid trunk's decode token recorded on the device (`Trunk::forward_token_on_device`): the residual stream, every residual add fused with the norm that follows it, each sub-layer's input fold fused with its quantize, the recurrent and attention sub-layers, the dense FFN (its activation, fold and quantize one dispatch), the final norm and the output head — in submissions of `ORANGU_HYBRID_FLUSH_LAYERS` layers, one readback of the logits. Off, each sub-layer and FFN is its own submission with the norms and adds on the host. Bit-identical output; the control arm. |
| `ORANGU_HYBRID_PREFILL_CPU` | unset (`1` turns it on) | `[orangu-server].prefill_backend = cpu` from the environment (it overrides `device` and `auto`): the Qwen hybrid trunk runs a multi-token pass on the CPU backend and single-token decode on the selected device. On the CIX P1 with the 27B ternary model: prefill 1.6 → 10.3 tok/s at 64 tokens (11.3 at 256), decode unchanged. The attention caches and the recurrent state move between the two through the accessors that already keep one copy current. |
| `ORANGU_TERNARY_IDOT_GROUPS` | `4` (`1..=16`) | Runs of `ORANGU_TERNARY_IDOT_ROWS` rows one workgroup of the integer-dot ternary kernels takes in sequence, so the lane's role constants, the launch and the drain are paid once per run of four. Measured on the Mali-G720: the `PTQ1_0` FFN gate projection 1070 → 901 µs, the down projection 774 → 708; eight hurt the down projection (too few workgroups). |
...
| `ORANGU_NORM_WG` | `128` (must be `64`, `128`, or `256`) | Workgroup size (thread count) of the default tree-reduce RMSNorm and RMSNorm+residual-add kernels. These run one `dispatch_workgroups(1,1,1)` workgroup over the whole `n_embd` row, so they are occupancy-starved; more threads shorten each thread's grid-stride loop and light up more of one work-group processor's SIMDs. Raised from `64` to `128` after measurement (halving to `32` had previously *doubled* the time, so these are compute/load-bound, not launch-bound): decode GPU-execution time dropped ~11% (35.4 → 31.5 ms/token) on real `E2B`/`RX 5500M` with byte-identical output; `256` was no better than `128` (deeper reduction tree, more barriers). Only affects the default (non-subgroup) norm path, and only rows the wide kernel below does not take. |
| `ORANGU_NORM_WIDE` | on (`0` turns it off) | The whole-row RMSNorm family (norm, norm+residual, norm+residual×scale) on its wide kernel: `vec4` loads, every thread's loads issued straight-line and waited for once, held in registers for the rescale, a two-round shared-memory reduce. Taken for any row width that is a multiple of four; other widths keep the grid-stride kernel `ORANGU_NORM_WG` sizes. The grid-stride kernel spent its time in a serial chain of one-at-a-time loads, and measured 13 µs a dispatch where the wide one measures 3.5 — a decode layer runs five of these. `0` is the control arm of a sweep. |
| `ORANGU_SPLIT_PASSES` | unset (off) | Records a fused decode layer the old way, one compute pass per recording function (about ten per layer) instead of one pass per layer. A pass boundary costs the device several microseconds on top of the dispatch it precedes, and the merge is worth about a quarter of decode throughput on the model this was measured on; this flag exists so that number can be re-measured. Inside a pass the driver still orders dependent dispatches, so the two are the same computation. |
| `ORANGU_LAYER_PASSES` | unset (off) | Records each fused decode layer in its own compute pass instead of carrying one pass across a whole submission chunk of layers. The first dispatch of every layer used to measure about 6 µs more than the same kernel elsewhere — the cost of the pass boundary it followed — and nothing between two layers of a chunk needs the encoder. `1` is the control arm of a sweep. |
| `ORANGU_HEAD_WIDE` | on (`0` turns it off) | The per-head kernels after the Q/K/V projections on their wide forms: the Q norm+RoPE with the head loaded straight-line into `vec4` registers and a two-round shared reduce, and one dispatch for the whole key/value epilogue — K's norm and RoPE, V's norm, and both rows' write into the KV mirror — where four dispatches ran before. Taken for heads up to 512 wide and a multiple of four, on an `f16` or `f32` KV cache; a `q8_0` cache, a layer that borrows its V from K, and projection biases keep the separate dispatches. Same arithmetic in the same order for the rotation; output is byte-identical on the served model. `0` is the control arm of a sweep. |
| `ORANGU_NORM_PAIRS` | on (`0` turns it off) | The attention post-norm with its residual add and the FFN norm that reads its result as one dispatch, wherever the wide norms apply: the first result stays in registers for the second reduce and is stored once, for the FFN's own residual. `0` runs the two whole-row dispatches, the control arm of a sweep. |
| `ORANGU_ADD_NORM_PAIR` | on (`0` turns it off) | The norm pair on a layer **without** an attention post-norm (the llama family, `phi3`, `mistral3`): the residual add and the FFN norm as one dispatch, with the same 8-bit epilogue for the gate and up projections. Measured level with the two dispatches on SmolLM2-360M and Llama-3.2-1B (a merged dispatch is a few microseconds; the token is milliseconds) — kept for the count, and `0` is the control arm. |
| `ORANGU_FFN_GATE_UP` | on (`0` turns it off) | A layer's gate and up projections as **one** decode dispatch over `[n_embd, 2 * n_ff]` where the checkpoint stores the two tensors adjacently with one type (`ffn_gate` then `ffn_up`, which a name-ordered file does; `phi3`'s single `ffn_up` tensor always) — a view over the bytes both already occupy, placed on the device once with the halves bound as sub-ranges of it, so nothing is uploaded twice. One dispatch's fixed cost instead of two, and a longer dispatch on a small model whose `[960, 2560]` matrices could not fill the card. Output is unchanged: the rows are the same rows in the same order. `0` runs the two dispatches, the control arm. |
...
| `ORANGU_ATTN_REDUCE_NARROW` | unset (off) | Runs the split-k attention *merge* on its original 64-thread kernel, a rolled loop of one dependent load per partial per element, instead of the 256-thread one that issues its loads four at a time and stages the softmax weights in shared memory once. The merge is the same arithmetic in the same order; the flag is the control arm of its measurement (+3% at short context, +10% at depth 1024 on the model this was measured on). |
...
| `ORANGU_DECODE_CHUNKS` | unset (integer; set, `3` equal chunks) | How many `queue.submit()` calls one decode step's layer loop is split across. `1` records the whole token and submits once (the historical behaviour); `> 1` submits the first `chunks - 1` groups of layers as soon as they are recorded, so the GPU starts executing them while the CPU is still recording and validating the later ones — overlapping the CPU-side submission cost with GPU execution instead of serialising it. Clamped to `1..=n_layers`. On real `E2B`/`RX 5500M` this raised decode throughput from 14.4 tok/s (`1`) to 18.8 tok/s (`7`, the default), with byte-identical output; `35` (one submit per layer) reaches 19.4 tok/s but adds per-submission overhead for a marginal gain. **Unset, the split is no longer equal:** the default is the geometric split `ORANGU_DECODE_FIRST_CHUNK` describes; setting this restores equal chunks. |
| `ORANGU_DECODE_FIRST_CHUNK` | `2` (integer; `0` = equal chunks) | Layers in a decode step's *first* submission; each following chunk is three times the last. Only the recording ahead of the first submission is host time the device cannot hide, so it carries two layers; the later chunks grow so the submissions stay few. Measured +4% against equal thirds on the model this was tuned on. Two rather than one: a single-layer first burst after a pause, when the card has parked its clocks, was measured holding a whole request at a third of its rate; two layers never did. The split applies to every family's fused decode chain (the llama family, `phi3`, `mistral3`, `muse-glimmer` and gemma), which used to submit the llama family's token once: SmolLM2-360M 93 → 139 tok/s, Llama-3.2-1B 99 → 130, 3B 58 → 65. |
| `ORANGU_PREFILL_FUSED_ATTN` | on (`0` turns it off) | A prefill layer's whole pre-attention half as one recording — the Q/K/V projections, the per-head Q/K norms, RoPE, V's weightless norm, the KV-cache write for the whole batch, and attention itself — with nothing between the projections and attention reaching the CPU, and attention's output left on the device for the post-attention chain. `0` takes the step-by-step path (projections read back to the CPU, norms and RoPE on the CPU, a per-token cache push, then a separate attention dispatch) and with it the whole host-orchestrated prefill: every layer's result comes back to the CPU. It was off for a while — measured as a loss against a prefill that computed only part of its output, then unable to run under the paged KV cache at all (a layer reading a donor's cache asked for pages past the sequence's reservation) — and is on now that both are fixed: 163/176/182 → 251/303/354 tok/s at 128/512/2048 prompt tokens with the device-resident stream below, output byte-identical. |
| `ORANGU_MAX_TOKENS_PER_SUBMISSION` | `512` on the Vulkan backend, `256` elsewhere (integer) | The token **stripe** of one prefill submission: a chunk wider than this runs its chains in stripes of it. Raised to 512 for `wgpu` once the integer-dot GEMMs' 128-token tiles existed (the same kernel at 256 tokens runs at three quarters of its 512-token rate) and every element-wise dispatch in the chains had become grid-stride, so nothing bounds the stripe but memory; before that a stripe was capped by the device's workgroups-per-dimension limit on the flat activation dispatch, which on the layers with a 12288-wide FFN cut a 512 stripe into 320 + 192. |
| `ORANGU_LLAMA_RESIDENT_PREFILL` | on (`0` turns it off) | On the `llama` family (Llama, Qwen2, Granite and the others that architecture serves), on `phi3` and on `mistral3`, whether a prefill chunk keeps its residual stream on the device: every layer's attention chain and post-attention chain recorded back to back, a few layers per submission, the K/V rows read back once per chunk and, for a chunk before the last, only after the next chunk's chains are submitted. The step-by-step path brought attention's output and the residual home at every layer, two readbacks and a host turn each with the device idle through them; measured on a 1B file this is 1068 → 1938 tok/s at 2238 tokens and 736 → 1263 at 156. Only whole-device runs take it (every layer on the first card, no NPU tier, no activation dump); `0` is the control arm. `ORANGU_RESIDENT_PREFILL_CHECK=1` runs every chunk on both paths and logs how far apart their residuals are (the root-mean-square difference over the chunk and the worst row): the two differ by the placement of their 8-bit activation roundings, ~1.5% RMS on the files measured, and a wrong parameter shows as whole rows off. |
| `ORANGU_PREFILL_LAYERS_PER_SUBMIT` | `4` (integer) | While the residual stream stays on the device, how many layers' chains are recorded into one submission. Each submission costs the host about a millisecond of driver validation and leaves the device idle for about as long, and a layer makes three chains; but a single submission for a whole chunk makes the device wait while the host records everything first, so `35` measured *slower* than `4` (311 vs 334 tok/s at 2048 tokens). `0` submits every chain on its own, the control arm; `1` is one per layer. |
| `ORANGU_PREFILL_KV_WAIT` | off (`1` turns it on) | While the residual stream stays on the device, a KV-owning layer's attention chain no longer waits for its key/value rows to come back to the host before the next layer is recorded: the positions are counted into the cache at once (the device pages already hold the values, which is all the next stripe and the next layer read) and the rows land in one mappable stage per chunk, filled after the chunk with a single wait. The host copy is what prefix reuse and slot save read, and it is complete by the time either can run. The per-layer wait had been the one place the device-resident prefill still drained the device: on a 42-layer model with 24 KV-owning layers it was 335 ms of a 158-token chunk. `1` restores the wait per layer, the control arm. |
...
| `ORANGU_MOE_HEATER` | unset (integer, workgroups) | A probe, not a setting: a spin kernel of that many workgroups after each routed feed-forward, to tell a dispatch slowed by an idle card's clock from one slowed by its buffers' placement (the latter was the case: the heater changed nothing, the region's placement changed everything). |
| `ORANGU_MOE_HEAD` | on (`0` turns it off) | On a mixture-of-experts model whose layers run on the card, the part of a layer between attention and the routed experts — the output projection, the post-attention norm and residual, the router's norm and logits, and the shared expert's whole MLP — runs as **two submissions per layer** instead of five separate calls each with its own upload and readback: the first returns the residual and the router logits the host needs to pick the experts, the second (the shared MLP) is left running while the host multiplies the routed experts, and its rows are collected afterwards. Attention's own result stays on the card for the first submission to read. A decode step was five submissions and four host round trips per layer; this makes it two and one, and a decode token one deep in its context takes the same position-parallel attention kernel the whole-step decode chain uses rather than the prefill kernel's one workgroup per query (0.9 → 0.3 ms a layer at a thousand positions on the page pool). `0` runs the five calls, the control arm. |
| `ORANGU_MOE_DECODE_BATCH` | off (`1` turns it on) | At decode, runs a mixture-of-experts layer's routed experts on the host as two parallel regions over every chosen expert's rows (gate and up, then down) instead of one per expert. Off because it measured slower on the models tried; kept as the arm of a sweep. |
| `ORANGU_EXPERT_REGION_RELEASE` | on (`0` turns it off) | Gives the expert streaming region back to the card after each prompt's prefill and takes it again at the next prompt that needs it — so a decode step has the region's memory for its KV pages and weights. Taking it again is refused, and that prompt's routed experts run on the host, when the card has less left than the region plus `ORANGU_EXPERT_REGION_HEADROOM_MIB` (256): a region created into a full card lands in host memory and reads across the bus at every dispatch, and what it displaces reads that way for the rest of the process — a card filled to its last page stops moving anything back. Measured on a 26B-A4B file on a 4 GiB card: with the region held across prompts, a deep prompt after a shallow one decoded at 7.3 tok/s for the rest of the process against 10.7 with it released. `0` holds the region for the process, the control arm. |
| `ORANGU_CLOCK_HOLD` | on (`0` turns it off) | On a model whose routed experts live in host memory, holds the card's clock up through a decode step. Such a step alternates between the card and the host — the attention or recurrent projections and the shared expert on the card, the routed experts on the host — and each host turn leaves the card idle for a few milliseconds, after which its governor parks the clock and every device call runs at a fraction of its rate (measured: the card at 500 MHz through a whole decode, more than half the token in device calls that would take a tenth of it warm). The holder is a one-workgroup spin on a **second queue** of the same card — a compute-only family where the card has one, its own hardware ring, so it never sits in the working queue's order — armed for the step and released after it. Measured on a 30B-A3B hybrid: 8.9 → 11.0 tok/s, the card at its full clock through the step and drawing ~17 W more. A dense model, whose step never leaves the card, arms nothing — nor does a split dense model, whose token is mostly the host's row dots: measured level to 12% behind there, the holder's power coming out of the host's share of a laptop's budget. `0` is the control arm. |
| `ORANGU_CONTEXT` | unset (tokens) | The context one request must fit on the device — the environment form of `--context` / `[orangu-server].context`; see "Fitting a context" above. Layers move to the host until the card has room for that many tokens of KV cache beside the weights it keeps. |
| `ORANGU_KV_POOL_BYTES` | an eighth of the machine's RAM, at least 2 GiB (bytes) | Host memory the paged KV pool may take — its rows live in system memory in full precision, the card holds a copy in its own width. A fraction of total memory rather than of what is free, so the pool is the same size on every run of a machine and larger on a larger one; the flat two gigabytes it replaced held a 26B file to 4736 tokens on a machine whose card could serve far more. The pool is also bounded by the card (`ORANGU_KV_DEVICE_BUDGET` below), and the startup line names the resulting capacity. |
| `ORANGU_MMQ_TOKS` | `0` (the rule; `16`, `32`, `64` or `128` fixes it) | The token tile of the dense integer-dot GEMM: the longest of 128, 64, 32 and 16 that pads the batch by at most a quarter. The full tile made a 156-token prompt 256 rows of work in every projection; the rule takes 64 there and 128 where the batch fills it. Measured on a 3B file: 504 → 647 tok/s at 156 tokens, 761 → 831 at 572, level at 2236; a fixed 32 is 28% slower than the rule at 572. |
| `ORANGU_KV_POOL_RESERVE_MIB` | `448` (MiB) | Device memory kept out of the KV pool's pages, and out of a requested context's placement, for what a prefill allocates as it runs — a chunk's pooled regions, the K/V readback stage, the attention scratch. A sweep knob: the prefill's transient set is what it is, and the answer to a card at its last page is to shrink that set (every layer of a chunk recycles its scratch), not to grow the reserve. |
| `ORANGU_KV_DEVICE_BUDGET` | on (`0` turns it off) | Sizes the KV pool's device pages from the memory budget the driver reports for this process (what it may still allocate without moving anything to host memory): the budget less the planned weights, the streaming region and a reserve for the prefill's transient scratch, never less than the quarter of the headroom below. On a 4 GiB card with a 1.9 GiB model that is 9344 tokens of context where the quarter gave 4736; with a 2.3 GiB model 5760 where it gave 3264; a model that fills the card keeps the quarter. `0` is the quarter alone, the control arm. |
| `ORANGU_SWA_FULL` | off (`1` turns it on) | Keeps a full-context device copy on the sliding-window layers instead of a ring of their window — see "Windowed layers keep a window" above. Costs what the ring saves (most of a gemma-4 file's KV bytes) and buys nothing but a comparison. |
| `ORANGU_KV_DEVICE_SHARE` | `25` (percent, 1–95) | The floor of the pool's share of the device headroom, and the whole of it where the driver reports no budget. Past what the budget rule allows it is harmful, not merely wasteful: a pool that takes the card past the driver's budget has part of the process moved into host memory and read across the bus at a fraction of the rate. |
| `ORANGU_TAIL_HOST` | probed (`0` device, `1` host) | Whether the vocabulary projection runs on the host after an `n_embd`-sized readback or on the card before an `n_vocab`-sized one. Unset, the server times a block of consecutive decode steps on each arm, the device's first, takes the host only when it is 10% faster, and asks again every 2048 steps. The block matters: arms alternating step by step let a host step park the card's clock and the device step after it was timed at the parked clock, so the host won by its own presence — and a whole process then ran with the card idling between tokens. `ORANGU_TAIL_TRACE=1` prints each decision. |
| `ORANGU_VULKAN_MEMORY_HINT` | blocks of at most 16 MiB (`usage` for 8–64, `performance` for 128–256, or an integer for that many MiB) | The block size the Vulkan backend's allocator packs small buffers into. A block stays with the card while one live allocation sits in it, and the chains' per-call uploads and scratch churn: with 64 MiB blocks a 4446-token prompt on a 1.9 GiB model held 1280 MiB of them with 383 live. Every large buffer (a weight chunk, the streaming region) is its own allocation either way; the difference is what the activation arenas and the KV pool's per-layer buffers leave reserved and empty — ~400 MiB of a 4 GiB card at the larger size, which on a model that fills the card was the difference between its streaming region fitting and the driver moving the KV pages out. |
| `ORANGU_VRAM_REPORT` | unset (`1` turns it on) | Prints the backend's live device allocations grouped by label, largest first, and its allocator's memory blocks by size class with what is live in them, after the KV pool is placed and with every `ORANGU_GPU_TIMESTAMPS=ops` report — what is competing for the card when a kernel that read fast an hour ago reads slowly now. The weight arena also reports each chunk it opens, with the plan it was cut to. |
| `ORANGU_KV_TRACE` | unset (`1` turns it on) | Prints a sequence's block table each time it grows — which of the pool's pages it holds, in order — and every per-request contiguous KV mirror the device builds, with its size and the path that asked for it. A request served from the pool builds none past the one-row scratch; a mirror of hundreds of MiB beside a pool is a request the pool could not table. |
| `ORANGU_MUSE_FUSED` | on (`0` turns it off) | Whether a `muse-glimmer` model runs its decode step as one submission per device through the fused layer chain (the output norm and the logits projection behind it), and its prefill through the two fused chains. The chain takes what this architecture adds to the llama block: no rotation on a full-attention layer, a sigmoid gate on attention's output projected from the same normed input (`attn.gate`, `attn.gate_mul` in the per-dispatch stamps), and the post-norms' own epsilon. Measured on a model split between a card and an integrated GPU: the server's CPU per generated token 265–309 → 21–26 ms, decode +36%; prefill of 328 tokens 1.4 → 5.0 tok/s. `0` restores the step-by-step path — one `Backend` call per matmul, ~200 submissions a token — the control arm. |
| `ORANGU_PREFILL_MMQ` | on where the device accelerates the packed 8-bit dot (`0` turns it off, `1` forces it) | The prefill's `Q4_K` projections on the integer-dot GEMM: the activation rows are quantized to 8 bits per 32-element block once per stage, the 4-bit weights are read as bytes, and each dot instruction does four multiply-adds — against one per element on the float kernel, which dequantizes every weight first. Taken for any `Q4_K` (and, see the next row, `Q6_K`) projection of 32 or more tokens whose width is a multiple of 256 and height of 64. **Not bit-identical**: the 8-bit activation changes the low bits, the way it does on every engine with this kind of kernel — a 1,563-token prompt's greedy answer differed in wording at the first sentence while a 2,846-token one did not. `0` keeps the float kernels everywhere, the control arm and the way to get the float path's exact numbers back. Every architecture's prefill takes it, not only the fused chains: the generic matmul batch every CPU-orchestrated model goes through picks the integer-dot kernel for each projection it applies to and the float kernel for the rest, in the same submission. Unset, the default is *per device*, from the driver's own word on whether the packed dot is a native instruction: where it is not (an integrated GPU holding a split model's overflow layers, say) the emulated dot loses to the float kernel — measured 16–18% slower at a 128-token prefill on a 30B dense model — so that device keeps the float GEMM while the card beside it uses the integer dot. `/props` reports which under `gpu.flags.prefill_mmq`; `1` forces the kernel on regardless, for the control arm of exactly that measurement. |
| `ORANGU_MMQ_STUB` | unset (off) | A probe, not a setting: builds the integer-dot GEMM with its dots replaced by one xor, so the kernel's time splits into the arithmetic and everything around it. The output is meaningless under it. |
| `ORANGU_PREFILL_MMQ_Q6K` | on (`0` turns it off) | The `Q6_K` projections (`ffn_down` on most layers of a `Q4_K_M` file) on the integer-dot GEMM too. Its blocks are 210 bytes, so a row's odd blocks start two bytes into a word and the staging reads them a word at a time and unpacks to bytes; 21% faster than the float kernel on the device clock, about 3% on a whole prefill. `0` keeps that projection on the float kernel while the `Q4_K` ones stay on the integer dot. |
| `ORANGU_PREFILL_MMQ_WIDE` | on (`0` turns it off) | The integer-dot GEMMs' **wide tiles**: a 128 × 128 (or 64 × 128) output tile per workgroup, the reduction staged 32 elements at a time rather than a whole 256-element super-block, and 128 (or 64) accumulators per thread — each staged byte serves 32 tokens × 4 rows where the narrow 64 × 32 tile served 2 × 8. Chosen per projection by shape (`ORANGU_MMQ_WIDE_MIN_WG`, default 32: the half-height tile needs that many workgroups, the full one twice as many; a projection too narrow for either keeps the narrow tile). Together with the unchecked indexing below, the `Q4_K` FFN GEMM at 512 tokens went from 3.4 to 5.1 TFLOP/s-equivalent on rotating weights and the `Q6_K` one from 1.9 to 4.0. `0` is the control arm. |
| `ORANGU_CHECKED_MMQ` | unset (off) | Builds the integer-dot GEMM kernels **with** the shader compiler's injected array-bounds clamps. They are built without them by default, whatever `ORANGU_TRUSTED_SHADERS` says: every index those kernels form is bounded by construction (the grid from validated dimensions, the activation buffer padded to whole tiles, the one guarded store guarded), and the clamps — a compare and a clamp per shared-memory read on a kernel already at the register limit — cost 46% of the wide tile's time. `1` puts them back for a bisect. |
| `ORANGU_MMQ_WIDE_MIN_WG` | `32` (integer) | The workgroup floor behind `ORANGU_PREFILL_MMQ_WIDE`'s tile choice. |
| *(the other weight types)* | — | The wide tiles exist for `Q5_K`, `Q8_0`, `Q4_0`, `Q4_1`, `Q5_0`, `Q5_1`, `Q2_K`, `Q3_K`, `IQ4_XS`, `IQ3_S` and `IQ2_S` too (`vulkan_shaders::shader_source_mmq_wide_bytes`: one generator, a staging routine per type, each row's sub-block staged as 32 bytes and two scale terms so one dot loop serves them all; the types whose scales cover sixteen values — `Q2_K`, `Q3_K`, `IQ2_S` — stage two pairs and take the dot per half; the lattice types look their bytes up in the codebook buffer at staging), built on first use per type rather than at start-up. They have no narrow tile, so a projection too small for the half-height tile (the 256-wide K/V and per-layer-embedding projections) stays on the float kernel. Measured on rotating weights at 512 tokens: 3.4–5.8 TFLOP/s-equivalent against the float kernel's 0.7–1.5, which is what made every non-Q4_K file's prefill a third of the Q4_K_M one's. |
| `ORANGU_PREFILL_PLE_INPUTS` | on (`0` turns it off) | gemma4's per-layer-embedding inputs for a prefill chunk computed **on the device** — the projection of the scaled embeddings as one GEMM, then one dispatch per (token, layer) row for the scale, RMS-norm, gathered-embedding add and scale, written layer-major so every layer's stage reads its operand as a slice — recorded ahead of the chunk's first chain. `0` computes them on the host as before: a device GEMM read back (18 MiB at 512 tokens), normed and added on the CPU, and uploaded again a layer at a time, with the device idle throughout. |
...
| `ORANGU_HOST_MATMUL_KIB` | `6144` | Weight bytes, in KiB, below which a **decode** matmul (`n_tokens == 1`) runs on the host instead of the device; `0` switches the rule off and is the control arm a sweep needs. A device matmul costs a large fixed amount before it computes anything — building a command buffer, handing it to the driver, waiting for the device to take it — and that cost is several times the host's, so the two backends' cost lines cross: above the crossing the device's bandwidth wins, below it the device's fixed cost does. The threshold is compared against the **whole batch's** weights, not one op's, because the fixed cost is per submission and a batch pays it once. **It is machine-dependent** — the card, the driver and the clock all move it — so measure it where the model will run: `cargo test --release --bin orangu-server decode_matvec -- --ignored --nocapture` prints the crossing directly. The default is this project's own card, where the probe put the crossing at 5.3–7.7 MiB and an end-to-end sweep agreed (4 MiB −6%, 6 MiB and 8 MiB within 1% of each other, 20 MiB −5%). Measured worth: +4.7% on `Ornith-1.5-35B-A3B`, +10.1% on `Llama-3.2-3B`, +0.3% (neutral) on `gemma-4-12B`, whose tensors are all above the threshold. Unlike most tuning here this **changes the numbers a model produces**, because a host matmul sums in a different order than a device one; it is a routing choice between two backends the engine already trusts, the same choice `matmul_host_fallback` makes for a type the device has no kernel for. | **It also decides whether a mixture-of-experts layer overlaps its shared and routed branches** (`arch::moe_overlap`): that `rayon::join` pays only when the two are on different processors, so a shared branch this rule sends to the host means no join — otherwise the two become fan-outs competing for one thread pool. Worth about +2-5% on `Ornith-1.5-35B-A3B` and neutral on `Ling-3.0-tiny`.
| `ORANGU_NPU_WEIGHTS_GB` | `1.5` | How many GiB of requantized weights the Rockchip RKNPU backend may hold on the device; `0` keeps every weight on the CPU and is the control arm. **The single biggest lever on what `backend = npu` is worth**, because everything that does not fit runs on the CPU exactly as before. The ceiling is the device's, not the machine's: an RK3588 stops handing out memory near 2 GiB across all contexts — measured, a loop allocating 4 MiB at a time is refused at 2048 MiB, and 254 contexts holding a weight plus scratch each are refused at 2034 MiB. The default leaves headroom under that, because the allocation that fails when it runs out is as likely to be scratch for an already-resident weight as a new weight. Measured on an RK3588 prefilling a fresh 1976-token prompt through `gemma-4-E2B-it` at `Q4_K_M`, against `backend = cpu`'s 16.4 tok/s: `0` → 16.3 tok/s (0.99×, the wrapper costs nothing), `1.0` → 19.4 (1.18×), `1.5` → 21.2 (1.29×), `1.75` → 22.2 (1.35×). Still climbing at the top of that range, so the limit is the device and not diminishing returns; the quarter-gibibyte between 1.5 and 1.75 is what a model with a vocabulary-sized output projection needs for scratch at a wide rung. Running out is handled — the weights already placed stay, the rest run on the CPU — and says so once. |
| `ORANGU_NPU_MODE` | `int8` | Which arithmetic the RKNPU is asked for: `int8` (`int8 × int8 → int32`, with a symmetric scale per output channel on the weight and one per token on the activations, all recombined on the host) or `fp16` (`float16 × float16 → float32`). int8 is the default on throughput and density: measured at `K = N = 2048`, 926 GFLOP/s at 64 tokens and 1190 at 512, against fp16's 265 and 266 — 4.5× — at half the bytes per weight, which doubles how much of a model fits under `ORANGU_NPU_WEIGHTS_GB`. `fp16` needs no requantization of a GGUF weight at all and is the fallback if int8 ever costs visible quality. The platform's mixed mode (`fp16 × int8`) and every int4 variant are rejected by this runtime as `unsupported matmul dtype ... in this platform`, so there is no third option on RK3588. An unrecognized value warns and uses int8 rather than silently selecting something. |
| `ORANGU_NPU_MIN_TOKENS` | `16` | The fewest tokens in a matmul worth sending to the RKNPU; anything narrower goes to the CPU backend, and `0` sends everything. **This is what makes `backend = npu` not a regression.** The device's fixed cost is about 0.4 ms per call, so at one token it manages 20 GFLOP/s against 1190 at 512, and the CPU — which reads the `Q4_K` weight straight out of the mapped file rather than a requantized copy — wins a narrow call outright. Overriding `Backend::matmul_decode` is the semantically right statement and was **not sufficient**: a decode step reaches `matmul` too, and with only that override in place `backend = npu` decoded at 2.15 tok/s against `cpu`'s 5.27. This width rule closed it to 5.12. Same shape of argument as `ORANGU_HOST_MATMUL_KIB` above, applied to a device whose crossing is in tokens rather than bytes. |
| `ORANGU_RKNN_LIB` | unset (`/usr/lib/librknnrt.so`, then `/usr/lib64`, `/usr/local/lib`, `/usr/lib/aarch64-linux-gnu`, then the bare soname) | An explicit path to Rockchip's RKNN runtime, for an SDK installed somewhere the search does not guess. It **replaces** the search rather than being tried first, the same rule `ORANGU_NPU_LIB` has for the NOE stack and for the same reason: a probe pointed at one library must not quietly succeed against another that happens to be installed. Honoured identically by `orangu-server system`'s report and by the backend that opens the library, because both go through one resolver — a machine where those two named different files would be a machine whose report is a lie. |
| `ORANGU_EXPERT_DOT_TILED` | on (`0` turns it off) | Whether a routed expert's **prefill** projection — many tokens against one expert's rows, the call that was 87% of a mixture-of-experts model's prefill CPU — takes the token-tiled integer-dot kernel. The kernel loads each 32-byte weight block once and dots it against four tokens with the products kept in registers, one horizontal reduction per output; the kernel it replaces reduced every block for every token and wrote the partial sums back to memory. Same integer products, so the sums match; only the float order of the block scales differs (within 1e-4 of the per-block reduction). Measured on the 26B-A4B mixture: prefill 37.5 → 43.8 tok/s at 128 tokens, 37.4 → 47.1 at 512; decode untouched — one token takes the single-token kernel, not this one. `0` restores the per-block reduction, the control arm. Takes effect only where the host has the vector extensions the kernel is built for; elsewhere the per-block kernel runs regardless. |
| `ORANGU_SSM_FANOUT` | on (`0` turns it off) | Whether a selective state-space block (the `nemotron_h_moe` recurrence) updates its heads in parallel. Each head owns a `[head_dim, state]` matrix and its own slice of the output and reads only its group's `B`/`C`, so the update is a fan-out with nothing to synchronize; before, the sixty-four heads ran one after another on the calling thread while the rest of the pool idled between two expert stages. Bit-identical — the per-head arithmetic and its summation order are unchanged, only which thread runs which head. Measured on a 30B-A3B hybrid: `recurrent.delta` 16.4 → 7.7 ms per token, out of ~145. `0` runs the heads serially, the control arm. |
| `ORANGU_Q5_UNPACK_VECTOR` | on (`0` turns it off) | Whether the host `Q5_0`, `Q5_1` and `Q4_0` kernels unpack a 32-element block with the AVX2 form — one byte shuffle and one compare give every lane its fifth bit — instead of the scalar loop that shifted `qh` once per lane and could not be vectorized by the compiler. Exact: the same bytes come out, checked lane by lane against the scalar loop. Reaches the routed experts' decode dot, the many-token prefill dot, and any of those types on a host-side projection; on a file whose experts are `Q5_0` the host `Q5_0` rate was 20 GB/s against `Q8_0`'s 36, and the unpack was the difference. `0` keeps the scalar loop, the control arm. Only on x86-64 with AVX2; elsewhere the scalar (or NEON) form runs regardless. |
| `ORANGU_EXPERT_WILLNEED` | on (`0` turns it off) | Whether a mixture-of-experts layer asks the kernel to start reading every expert it has just selected — across the gate, up and down tensors — before the first of them is touched. **Only acts on a model whose routed experts are larger than the machine's memory** (the startup line `[experts] … streamed from the disk` says when); on a model the page cache holds, the hint would find every page in place and cost a page walk per expert per layer, so it is skipped. Without it, each expert is faulted in page by page from under the dot that needs it: a thread per expert, each fault a small read the drive answers one at a time, and the kernel's read-ahead never sees a pattern it recognizes. With it the whole selection is one batch of large sequential reads the device can queue, and the down projection's bytes arrive while the up projection is computing. `0` leaves the reads to the page faults, the control arm. Ignored by a read route that bypasses the page cache (`ORANGU_EXPERT_READ=direct`). |
| `ORANGU_MATMUL_MIN_ROWS` | `64` | The fewest output rows a thread-pool task takes when a host matmul fans out over its rows (`with_min_len`, a floor rather than a chunk size — `rayon` still splits down to it and no further). One row per task is a few hundred nanoseconds of arithmetic inside a job that has to be created, possibly stolen and joined; on `Ornith-1.5-35B-A3B` **40% of all CPU samples were thread-pool machinery** and 24% were `sched_yield` alone, after the host-routing rule multiplied a decode step's parallel regions. Worth +2 to +4%, and it takes the pool's share from 40% to 34%. `1` disables the floor and is the control arm. A flat number rather than one scaled to the pool, and that was measured, not assumed: scaling it as `out_dim / threads` is **6.7% slower than no floor at all**, because a routed expert's projection already sits inside an eight-wide fan-out over experts and this call cannot see it. Machine-dependent, like every other threshold here. **Bit-identical at any value** — each output row is computed and written independently, so this changes only which worker does what. |
| `ORANGU_DECODE_STAGES` | unset (off) | Accumulates the **elapsed** time of each forward-pass stage, drained by `GET /decode-stages` and printed by `orangu-bench` as its `stages` lines. Answers what a CPU profile cannot: a stage running alone on one core costs the whole of its wall time while the same cycles spread over sixteen cores cost a sixteenth, and a stage parked in a GPU driver waiting for a submission barely samples at all. `forward`, `attn`, `ffn.router` and `ffn.routed` are timed in code every architecture goes through, so they are reported for any model; the finer stages are timed per-architecture and an architecture without them reports the remainder under `other`; a token recorded as one device submission (the Qwen hybrid trunk on Vulkan) reports its host-side recording as `device.record` and the submissions, the wait and the readback as `device.submit`. Off by default because two clock reads and two atomic adds per stage entry are not free at the rate a decode step enters them, and a measurement that perturbs what it measures whenever it is linked in is worse than none. A diagnostic; no effect on the computation. |
...
| `ORANGU_CPU_TIMESTAMPS` | unset (off) | Logs, per decode step, how its host time splits: `since-return` (from the previous step's token to this step's start — the generate loop's own share), `record` (building the step's commands), `submit+wait` (the submissions and the wait for the device), and each intermediate submission's own call time. For a prefill chunk it logs the host's phases instead — `stage` (the token and per-layer embeddings gathered), `ple` (the per-layer inputs recorded), `record` (the layers' chains), `submit`, `fill-previous` (the wait for the *previous* chunk's K/V rows and their fill into the host cache — with the device pipelined this is most of the chunk's wall time, which is the point), `land` (the last chunk's residual read back) and `kv`. Read with `ORANGU_GPU_TIMESTAMPS=ops`'s `idle before` to see how much of the host's turn the device waits for. |
| `ORANGU_GPU_TIMESTAMPS` | unset (off; `1` or `ops`) | Logs a per-decode-step GPU timing breakdown to stderr — the per-layer-embedding (PLE) projection, the sum/average/slowest across all model layers, and the output-norm-plus-`lm_head` tail, in milliseconds. Also logs a `[gpu-op-breakdown]` line splitting each token into **qkv-side** (Q/K/V matmuls + norm/RoPE + KV write), **attention** (split-k), and **ffn-side** (wo/gate/up/down matmuls + their norms + GELU/mul + PLE + copies) — so matmul vs attention vs overhead is measured, not estimated. Requires an adapter with `TIMESTAMP_QUERY` and `TIMESTAMP_QUERY_INSIDE_ENCODERS`; a diagnostic, no effect on the computation. **Unavailable on a split model**: a query set belongs to one device and a split resolves none, so `GET /gpu-timings` answers `{"enabled": false, "timings": null, "unavailable": "split"}` — the reason is named rather than left as an empty result, since a client that reports nothing when it receives nothing makes a split run look like one whose GPU stages cost nothing. `--flamegraph` still profiles the CPU side. **`ops`** additionally stamps every dispatch of the step from inside its compute pass and prints a `[gpu-ops]` block per token: one line per kind of dispatch (`ffn.down`, `attn.norm`, `ple.gate`, …) with its total, count and average — the cost *as the step pays it*, the barrier and any pass boundary in front of the dispatch included, which is what separates a slow kernel from an expensive schedule around a fast one. The header line also carries `idle before`, the device's gap between the previous step's last dispatch and this one's first — the host's turn as the device sees it — and a `memory:` line with the allocator's block and allocation counts. `ops` replaces the per-layer brackets of `1` rather than adding to them. Needs `TIMESTAMP_QUERY_INSIDE_PASSES` as well; says so and prints nothing otherwise. |
...
was to send a request and read the answer. `/props` reports the same value as
`role`.

A coordinator starts a **real `--review` process** for the review role rather
than labelling a request on another one, so `Mode review` on the banner is what
an `/auto_review` run is served by. `all`, `code` and `explorer` do share a
process — nothing about them differs that a request cannot carry — and a
request served under one of those on another's process says so on its own
completion line. A request served under
another role says so on its own completion line, which is how a run can be
checked rather than assumed:

```
orangu-server: [slot 0 as review] prompt 27 tokens in 1.50s (17.94 tok/s), generated 60 tokens in 2.41s
```

### Refusing a context the device cannot hold

The cheapest way not to lose a device is not to hand it work that loses it.
A request's KV cache is the one part of its footprint that scales with what
the caller sent, and past a point the device stops answering a submission
inside the driver's reset timeout — not because an allocation failed, but
because it is paging while it works. What follows is a reset, an `exit 75`,
and every other request on the process dying with the one that caused it.

So the size is refused where an answer is still free, at the HTTP boundary,
before a token is processed:

```
$ curl -s localhost:8100/v1/chat/completions -d @big.json
prompt (41833 tokens) plus max_tokens (512) needs 42345 tokens of KV cache,
more than the 16384 this server has room for on its device — send a shorter
prompt, lower max_tokens, or serve this model on a device with more memory
```

`400`, not `500`: retrying changes nothing and only a shorter prompt can
work, which is what the status code tells a client to do. `/auto_review` does
exactly that — a file whose prompt is refused is re-sent without its
whole-file section and with its diff bounded, and is reviewed from that.

The number is derived per machine, never written down for one. With the paged
KV pool on (the default) it is the pool's own token capacity, which
`KvPool::pages_within` computes from *this* device's headroom after the
weights and this model's KV geometry — so it rises with the card. Without a
pool it is the headroom figure the startup report already prints. With no
device at all there is no ceiling and nothing is refused. The line naming it
is printed at startup, next to the pool's own:

```
orangu-server: [kv] paged cache — 256 pages of 64 tokens (16384 tokens total …)
orangu-server: [kv] one request may hold up to 16384 tokens of context; a
longer prompt is refused rather than risking the device
```

A request the pool cannot promise room for is not refused on that account
alone — it falls back to a per-request contiguous cache, which is what every
request used before the pool existed, and keeps prefix reuse through
`slot_store`. It is refused only when it is also over the ceiling above.
...
The eight endpoints, their request and response fields, the Git integration
and the error codes are documented in the *HTTP endpoints* chapter, under
**File-lifecycle API** — one place for a contract three surfaces share (these
endpoints, `orangu`'s file tools, and its typed commands of the same names).

What is worth saying here is where the code lives. The operations are
`orangu::files`; the axum router that exposes them is
`orangu::files_http::router`, mounted by `orangu-server` beside its inference
endpoints and generic over a `WorkspaceState` that names the workspace root.
Both are library code rather than server-binary code precisely because
`orangu`'s own tools and typed commands call the same functions, so a tool
call and an API request cannot drift apart in fields, defaults or errors.
`orangu-coordinator` has no implementation of its own and proxies them.
