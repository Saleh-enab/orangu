...
| `ORANGU_LLAMA_RESIDENT_PREFILL` | on (`0` turns it off) | On the `llama` family (Llama, Mistral, Qwen2, Granite and the others that architecture serves), whether a prefill chunk keeps its residual stream on the device: every layer's attention chain and post-attention chain recorded back to back, a few layers per submission, the K/V rows read back once per chunk and, for a chunk before the last, only after the next chunk's chains are submitted. The step-by-step path brought attention's output and the residual home at every layer, two readbacks and a host turn each with the device idle through them; measured on a 1B file this is 1068 → 1938 tok/s at 2238 tokens and 736 → 1263 at 156. Only whole-device runs take it (every layer on the first card, no NPU tier, no activation dump); `0` is the control arm. |
