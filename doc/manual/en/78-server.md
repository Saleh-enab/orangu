...
| `ORANGU_CPU_TIMESTAMPS` | unset (off) | Logs, per decode step, how its host time splits: `since-return` (from the previous step's token to this step's start — the generate loop's own share), `record` (building the step's commands), `submit+wait` (the submissions and the wait for the device), and each intermediate submission's own call time. Read with `ORANGU_GPU_TIMESTAMPS=ops`'s `idle before` to see how much of the host's turn the device waits for. |
