...
| `ORANGU_VULKAN_MEMORY_HINT` | `usage` (`performance` for the control arm) | The block size the Vulkan backend's allocator packs small buffers into: `usage` is 8–64 MiB blocks, `performance` 128–256 MiB. Every large buffer (a weight chunk, the streaming region) is its own allocation either way; the difference is what the activation arenas and the KV pool's per-layer buffers leave reserved and empty — ~400 MiB of a 4 GiB card at the larger size, which on a model that fills the card was the difference between its streaming region fitting and the driver moving the KV pages out. |
| `ORANGU_VRAM_REPORT` | unset (`1` turns it on) | Prints the backend's live device allocations grouped by label, largest first, after the KV pool is placed and with every `ORANGU_GPU_TIMESTAMPS=ops` report — what is competing for the card when a kernel that read fast an hour ago reads slowly now. The weight arena also reports each chunk it opens, with the plan it was cut to. |
| `ORANGU_KV_TRACE` | unset (`1` turns it on) | Prints a sequence's block table each time it grows — which of the pool's pages it holds, in order. For telling a slow request apart by *where* its pages are from one that is slow for another reason. |
