...
| `ORANGU_DECODE_FIRST_CHUNK` | `1` (integer; `0` = equal chunks) | Layers in a decode step's *first* submission; each following chunk is three times the last. Only the recording ahead of the first submission is host time the device cannot hide, so it carries one layer; the later chunks grow so the submissions stay few. Measured +4% against equal thirds on the model this was tuned on. |
