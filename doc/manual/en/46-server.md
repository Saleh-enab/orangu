...
the `glm4moe` one above — an ordinary GQA-with-experts model that shares a
name with the supported `glm-dsa` and none of its graph) is printed greyed
rather than hidden: you can still
...
This section is the reference; the _Image generation_ chapter that
follows _Building a model_ is the walk-through — what to download, how
