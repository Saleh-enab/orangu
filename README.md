...
- GitHub-flavored Markdown rendering in the console: headings, emphasis, blockquotes, links, inline and fenced code, task lists, and aligned tables. Colours and table borders adapt to the active theme.

**Share what you produce.** Copy the latest assistant response as its original Markdown with `/copy`; export the output window, the last interactive or automated review report, a pull-request or issue summary, the activity statistics, or a duplicate-code report to a PDF in the workspace root (`/export console`, `/export review`, `/export auto review`, `/export pr`, `/export issue`, `/export statistics`, `/export duplicates`), or post a review straight onto an issue or pull request with `/comment <number> with review` / `with auto review`.

**Sessions and servers under your control.** Conversations are stored and resumable — `-r <uuid>` resumes one, `-r` alone picks one from a list (or starts a new one), `-l` lists every stored session as a table, `/session` lists and switches between them (or opens a directory as a new workspace), and `/prune` deletes them by id, workspace, or age. `/model` and `/server` switch model or configured server at runtime, `/information` reports which OpenAI-compatible and native endpoints the active server actually implements (plus the local knowledge-graph scan status), `/reload` returns to the configured model and server and starts a clean exchange, `/restart` re-execs orangu in place to pick up a freshly built binary without losing the session, and `/disconnect` drops the connection.
...
Run `/auto_review <file>` to review a single file (the whole file on `main`/`master`, or just its changes on a branch), `/auto_review <pattern>` — e.g. `/auto_review src/main/java/**` or `/auto_review *.rs` — to review every file matching a glob the same way, or `/auto_review all` to review every Git-tracked file in the project. `immediate` and `deep` combine with any form, or the bare command, in any order — add `immediate` to skip the pre-start phase and `deep` to start every file in Deep mode, e.g. `/auto_review all immediate`, `/auto_review src/main/java/** immediate`, `/auto_review deep <file>`, or `/auto_review deep all immediate` (Tab-completed and ghosted like the file argument). Like `/review`, the report is copied to the clipboard and reusable with `/export review` and `/comment ... with auto review`.

**`/create_patch` — turn review into working code.** After either reviewer finishes, `/create_patch` gives the coding model the latest actionable, deduplicated findings (including successful interactive-review feedback), has it verify them against the current tree, apply valid fixes, run focused checks, and report anything deliberately skipped. Review context has a fixed initial budget; overflow is cached in small category/path nodes that the model expands only when relevant, so a long review cannot flood the context window. It edits the working tree but never commits or pushes. If Git has unresolved merge, rebase, or cherry-pick paths, the same command also resolves every conflict (even when no review has run), stages the resolved paths, and verifies that no unmerged entries remain. Conflict paths are always supplied directly rather than compressed. The natural-language forms are `create patch` and `fix review findings`.
...
**Model coverage.** Text-in/text-out GGUF chat, completion, and embedding models — and one text-to-image model — across seventeen servable architecture families: Llama-style (`llama`, `qwen2`, `qwen3`, `mistral`, and the text backbones of Qwen3-VL and Qwen2.5-VL), Gemma (`gemma`/`gemma2`/`gemma3`/`gemma4`, dense **and** the routed-expert `gemma-4-26B-A4B` MoE, plus the embeddings-only `gemma-embedding`), Qwen3.5/3.6-MoE, Qwen3.5-family dense (`qwen35`, covering Qwen3.8 and Prism's Hadamard-folded `Ternary-Bonsai-2-27B`), Qwen3-Next, the Qwen4 preview (`qwen4exp`, covering Qwen3.8-Flash-Next — the Qwen 3.5 sub-layers and MoE on a four-stream hyper-connection residual, with indexer-selected block-sparse attention and an n-gram-hash per-layer embedding), DeepSeek-V4, GLM-5 (`glm-dsa`), GLM-5.3-Flash (`glm5next`), Kimi-K3, Phi-3/Phi-4-mini, Mistral 3, Muse-Glimmer (`muse-glimmer`), Inkling (`inkling`), Nemotron-H (`nemotron_h_moe` — blocks that are a single sub-layer each: a selective state-space mixer, an unrotated attention, or a squared-ReLU MoE FFN), and Ling 3.0 (`bailingmoe3` — three-in-four Kimi Delta Attention layers alternating with gated, rotated latent attention, over group-limited routed experts), and Qwen-Image (`qwen_image` — a dual-stream diffusion transformer served with its Qwen2.5-VL text encoder and VAE beside it, answering chat turns and `/v1/images/generations` with PNG, JPEG, GIF or WebP pictures, from a prompt or from an attached picture). Tensors are read as `F32`/`F16`/`BF16`, `Q8_0`/`Q4_0`/`Q5_0`, `MXFP4`, Prism's ternary `PQ2_0`/`PTQ1_0`, the K-quants (`Q2_K`-`Q6_K`), and the I-quants (`IQ4_XS` down through `IQ1_S` and the sub-1.5-bit `IQ1_XS`/`IQ1_XXS`/`IQ1_XXXS`). Weights are dequantized lazily from the memory-mapped file, so large models fit in modest RAM, and models split across shards load from every file.

**DeepSeek-V4.** `deepseek4` GGUFs such as `unsloth/DeepSeek-V4-Flash-0731-GGUF` run natively — four-stream hyper-connections with per-token Sinkhorn mixing, one shared key/value vector per token across all 64 query heads, per-layer compressed attention (whole 128-token blocks, or 4-token blocks picked by the model's own lightning indexer) layered on top of a sliding window, attention sinks, and hash-routed experts on the first layers, all on the CPU path.

**GLM with sparse attention.** `glm-dsa` GGUFs such as `unsloth/GLM-5.2-GGUF` run natively: multi-head latent attention in its absorbed form (the whole layer's keys and values are one compressed vector per token, and the query is pushed through the key-decompression matrix rather than the cache being expanded), a lightning indexer that scores every earlier position so each layer attends only the best `indexer.top_k` of them, leading dense layers, and sigmoid-routed experts with a shared expert.

**GLM-5.3-Flash.** `glm5next` GGUFs such as `unsloth/GLM-5.3-Flash-GGUF` run natively, and reuse almost every part of the two above. Three layers in four are Kimi Delta Attention and every fourth is absorbed latent attention, as in Kimi-K3 and Ling 3.0. There is no residual *vector*: the state between sub-layers is four parallel streams mixed by the same Sinkhorn hyper-connections DeepSeek-V4 uses, collapsed at the end by a plain mean. Nothing in the model rotates — the latent layers are position-free — and they pick what to attend with a lightning indexer that scores fixed *pools* of four positions rather than single ones, cutting on pool boundaries and always attending the query's own trailing partial pool. The FFN is a leading dense block, then sigmoid-routed experts (288 of them, 8 used) with a shared expert, under DeepSeek-V4's pre-activation SwiGLU clamp.

**Kimi-K3.** `kimi-k3` GGUFs such as `unsloth/Kimi-K3-GGUF` run natively: three layers in four are Kimi Delta Attention (a gated delta-net whose state is a fixed-size matrix, so those layers cost nothing per token of context) and every fourth is absorbed latent attention with a sigmoid output gate, on top of cross-layer residual attention (layers bank their input and later layers re-mix against those checkpoints), routed experts that run in a latent space narrower than the model, and the situ activation in place of SwiGLU.

**Draft sidecars.** `orangu-server list` also recognizes `dflash` GGUFs such as the DeepSeek-V4-Flash DSpark draft. These carry no token embeddings and no output projection — they exist to draft for a target model, not to be served — so selecting one serves the paired target model from the same Hugging Face repo instead, fetching it if the models directory doesn't have it yet.
...
**Serving.** OpenAI-compatible `/v1/chat/completions` (streaming and not), `/v1/completions`, `/v1/models`, `/v1/embeddings`, and — on a Qwen-Image (`qwen_image`) model — `/v1/images/generations`, alongside native `/health`, `/props`, `/slots`, `/metrics`, `/completion`, `/tokenize`, `/detokenize`, `/apply-template`, and a loopback-only `/v1/shutdown`. Requests are scheduled across configurable slots with continuous batching, a prefix cache, and durable slot persistence so a resumed conversation need not re-prefill. Deployment roles (`--all`/`--code`/`--review`/`--explorer`/`--embedding`, and `--image`, which a `qwen_image` model takes on its own) adjust slot counts, sampling defaults, reasoning suppression, and which endpoints are served at all.
