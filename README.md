...
**Model coverage.** Text-in/text-out GGUF chat, completion, and embedding models — and one text-to-image model — across seventeen servable architecture families: Llama-style (`llama`, `qwen2`, `qwen3`, `mistral`, and the text backbones of Qwen3-VL and Qwen2.5-VL), Gemma (`gemma`/`gemma2`/`gemma3`/`gemma4`, dense **and** the routed-expert `gemma-4-26B-A4B` MoE, plus the embeddings-only `gemma-embedding`), Qwen3.5/3.6-MoE, Qwen3.5-family dense (`qwen35`, covering Qwen3.8), Qwen3-Next, the Qwen4 preview (`qwen4exp`, covering Qwen3.8-Flash-Next — the Qwen 3.5 sub-layers and MoE on a four-stream hyper-connection residual, with indexer-selected block-sparse attention and an n-gram-hash per-layer embedding), DeepSeek-V4, GLM-5 (`glm-dsa`), GLM-5.3-Flash (`glm5next`), Kimi-K3, Phi-3/Phi-4-mini, Mistral 3, Muse-Glimmer (`muse-glimmer`), Inkling (`inkling`), Nemotron-H (`nemotron_h_moe` — blocks that are a single sub-layer each: a selective state-space mixer, an unrotated attention, or a squared-ReLU MoE FFN), and Ling 3.0 (`bailingmoe3` — three-in-four Kimi Delta Attention layers alternating with gated, rotated latent attention, over group-limited routed experts), and Qwen-Image (`qwen_image` — a dual-stream diffusion transformer served with its Qwen2.5-VL text encoder and VAE beside it, answering chat turns and `/v1/images/generations` with PNG, JPEG, GIF or WebP pictures, from a prompt or from an attached picture). Tensors are read as `F32`/`F16`/`BF16`, `Q8_0`/`Q4_0`/`Q5_0`, `MXFP4`, the K-quants (`Q2_K`-`Q6_K`), and the I-quants (`IQ4_XS` down through `IQ1_S` and the sub-1.5-bit `IQ1_XS`/`IQ1_XXS`/`IQ1_XXXS`). Weights are dequantized lazily from the memory-mapped file, so large models fit in modest RAM, and models split across shards load from every file.
